...
class MissionMonitorTests(unittest.TestCase):
    ...
    def test_snapshot_surfaces_outer_loop_action_branch_budget_and_promotion(self) -> None:
        ...
        self.assertIn("# DeepLoop operator console", rendered)
        self.assertIn("## Top summary", rendered)
        self.assertIn("operator_summary: PAUSED — DeepLoop needs an operator decision before it can continue.", rendered)
        ...
        self.assertIn("operator_loop: `status` -> `inbox` (only when needed) -> `resume`", rendered)
        self.assertIn("## Main operator loop", rendered)
